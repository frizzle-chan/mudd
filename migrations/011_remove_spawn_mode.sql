-- Migration: 011_remove_spawn_mode
-- Description: Remove spawn_mode column and enum, update resolve_entity function
-- Pickup behavior is now controlled by effects.pickup() in on_take templates

SET lock_timeout = '1s';
SET statement_timeout = '5s';

-- Drop spawn_mode column from entities table
-- squawk-ignore ban-drop-column
ALTER TABLE entities DROP COLUMN IF EXISTS spawn_mode;

-- Update resolve_entity() to remove spawn_mode from return type
...
-- Drop the spawn_mode enum type (must be after dropping column)
DROP TYPE IF EXISTS spawn_mode;
