...
class TestEffectsObserverMergeFrom:
    """Tests for EffectsObserver.merge_from()."""

    def test_merge_broadcasts(self):
        """Broadcasts are merged from other observer."""
        main = EffectsObserver()
        main.notify(BroadcastEvent(message="Main"))

        other = EffectsObserver()
        other.notify(BroadcastEvent(message="Other"))

        main.merge_from(other)
        assert main.broadcasts == ["Main", "Other"]

    def test_merge_grants(self):
        """Grants are merged from other observer."""
        main = EffectsObserver()
        main.notify(GrantEvent(entity_id="key1"))

        other = EffectsObserver()
        other.notify(GrantEvent(entity_id="key2"))

        main.merge_from(other)
        assert main.grants == ["key1", "key2"]

    def test_merge_currency_grants(self):
        """Currency grants are merged from other observer."""
        main = EffectsObserver()
        main.notify(GrantCurrencyEvent(amount=50))

        other = EffectsObserver()
        other.notify(GrantCurrencyEvent(amount=100))

        main.merge_from(other)
        assert main.currency_grants == [50, 100]

    def test_merge_destroy_flag(self):
        """Destroy flag is ORed from other observer."""
        main = EffectsObserver()
        other = EffectsObserver()
        other.notify(DestroySignal())

        main.merge_from(other)
        assert main.has_destroy is True

    def test_merge_does_not_copy_pickup_flag(self):
        """Pickup flag is NOT merged (handled separately)."""
        main = EffectsObserver()
        other = EffectsObserver()
        other.notify(PickupSignal())

        main.merge_from(other)
        assert main.has_pickup is False

    def test_merge_does_not_copy_drop_flag(self):
        """Drop flag is NOT merged (handled separately)."""
        main = EffectsObserver()
        other = EffectsObserver()
        other.notify(DropSignal())

        main.merge_from(other)
        assert main.has_drop is False
